...
namespace BaSys.Host.DAL.DataProviders
{
    public sealed class DataObjectProvider : IDataObjectProvider
    {
        ...
        public async Task<SelectItem?> GetSelectItemAsync<T>(T uid, IDbTransaction? transaction)
        {
        ...
        public async Task<SelectItem?> GetSelectItemAsync(string uid, IDbTransaction? transaction)
        {
            ...
            switch (_primaryKeyDbType)
            {
                case DbType.Int32:
                    ...
                    item = await GetSelectItemAsync<int>(intValue, transaction);
                    break;

                case DbType.Int64:

                    long.TryParse(uid, out var longValue);
                    item = await GetSelectItemAsync<long>(longValue, transaction);
                    break;

                case DbType.Guid:

                    Guid.TryParse(uid, out var guidValue);
                    item = await GetSelectItemAsync<Guid>(guidValue, transaction);
                    break;

                case DbType.String:

                    item = await GetSelectItemAsync<string>(uid, transaction);
                    break;
